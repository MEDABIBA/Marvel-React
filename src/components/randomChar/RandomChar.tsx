...
const RandomChar = () => {
    const [char, setChar] = useState<dataChar>()
    ...
    useEffect(()=>{
     ...
     const onChatLoaded = (char: dataChar)=>{
        setProcess('succes')
        setChar(char)
     }
     const updateChar = ()=>{
        ...
        getCharacter(id)
        .then((char: dataChar) => onChatLoaded(char))
     }
     const TryRandomBtn = (): void => {
        updateChar()
     }
        return(
            <div className="randomchar">
                       {setContent(process, View, char as dataChar )}
