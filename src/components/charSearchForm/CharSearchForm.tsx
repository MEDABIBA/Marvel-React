...
const CharSearchForm = (): JSX.Element => { 
    const [data, setData] = useState<dataChar[] | null>(null)
    ...
    const results: JSX.Element | null= !data ? null : data.length > 0 ? 
        <div className='valid-success'>
            ...
            <Link to={`/characters/${data[0].id}`} className="success-block-button">TO PAGE</Link>
        </div>
